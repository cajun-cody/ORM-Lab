...
def problem_one(request):
    ...
    honer_students = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
  
    for student in honer_students:
...
def problem_three(request):
    ...
    courses = Course.objects.filter(instructor__id=2)
    print("Courses:")
...
def problem_five(request):
    ...
    student = Student.objects.create(first_name='Cody',last_name= "Dugas",year= 2022,gpa= 4.0)
...
def problem_six(request):
    ...
    """ student_id = 11 """
    Student.objects.filter(pk=11).update(gpa=3.8)
